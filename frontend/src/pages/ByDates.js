...
class ByDates extends React.Component {
  state = {}
  ...
  constructor({ name, color }) {
    ...
    if (!this.state.dfLoading) {
      ...
      AuthLogData.getLoginEvents().then(df => {
        ...
        //df['eventdatetime'] = dfd.to_datetime({
        //  data: df['eventdatetime'],
        //  format:"%Y-%m-%dT%H:%M:%S"
        //});
        // Update the UI
        this.setState({ "df": df });
  ...
  render() {
    if (!this.state.df) {
      ...
      let count = {};
      for (let [date,] of utilityFns.getTwoTuples(df, "date", "date")) {
